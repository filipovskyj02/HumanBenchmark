...
function getCookie(name) {
 ...
 function deleteCookie(name) {
  ...
  if (!validateForm(email,password,name,repeatPassword)) return;
  
  const hashedPassword = sha256(password);
  
  const data = { email: email, password: hashedPassword, name: name };
...
function toggleFormVisibility(form) {
  ...
  function registerUser(data) {
    ...
    .then(data => {
      ...
      document.getElementById("message").innerHTML = 'Account succesfully created !';
      setCookie('id_player', data.id_player, 7);
      setCookie('name', data.name, 7);
      console.log('Success:',data.id_player);
